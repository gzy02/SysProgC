...
char *create_dir(fnode *node, const char *base_fname)
{
    char dir_path[256];
    sprintf(dir_path, "%s/%s", node->tmp_dir, base_fname);
...
{
    char *dir_path = create_dir(node, base_fname);
...
fnode *init_fnode(int nInput, const char *argv[], int nOutputs, const char *tmp_dir, const char *output_dir)
{
    ...
    {
        node->files[i].fname = strdup(argv[i]);
        ...
        // free(create_dir(node, argv[i]));
        // create_files(node, argv[i], nOutputs, node->files[i].inter->file_paths);
    }
    struct stat st = {0};
